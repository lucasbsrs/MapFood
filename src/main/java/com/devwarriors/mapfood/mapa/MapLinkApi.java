...
public class MapLinkApi {
	...
	private static final String PATH_AUTENTICACAO = URL_API
	...
	public String obterToken() {
		...
		Response response = ClientBuilder.newClient().register(feature).target(PATH_AUTENTICACAO)
	...
	public SolucaoRota retornaSolucaoDeRotaPorId(String id) {
		...
		Response response = ClientBuilder.newClient().target(URL_API).path(String.format(PATH_GET_SOLUCAO, id))
				.request(MediaType.APPLICATION_JSON).header("Authorization", "Bearer " + obterToken()).get();
	...
	public String criaProblemaDeRota(RotaIndividualDto problemaRota) {
		...
		Response response = ClientBuilder.newClient().target(URL_API).path(PATH_POST_PROBLEMA)
				.request(MediaType.APPLICATION_JSON).header("Authorization", "Bearer " + obterToken())
